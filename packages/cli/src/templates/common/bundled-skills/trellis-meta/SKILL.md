...
- `.trellis/`: workflow, config, tasks, spec, scripts, bundled runtime agents, and runtime state.
...
- `references/local-architecture/session-memory.md`: Raw platform conversation recall through `trellis mem`, separate from task authority.
...
- `references/customize-local/change-context-loading.md`: Change how tasks, specs, hook context, channel inbox messages, and `trellis mem` recall are loaded.
...
- `.trellis/workflow.md` is the local workflow source of truth; its single `[trellis-continuation]` block owns active-task resume semantics. Its initial content was selected from a workflow template (built-in `native`, `tdd`, `channel-driven-subagent-dispatch`, or a marketplace template) at `trellis init` time and can be re-selected via `trellis workflow --template <id>`. Missing `.trellis/agents/<name>.md` files referenced by the active template trigger a non-blocking stderr warning pointing at `trellis update`.
- `.trellis/config.yaml` is the project-level Trellis configuration entry point. It hosts task lifecycle hooks (`hooks.after_create` / `after_start` / `after_finish` / `after_archive`), task archive policy (`task_auto_commit`), channel worker guard (`channel.worker_guard.idle_timeout` / `max_live_workers`), Codex dispatch mode (`codex.dispatch_mode: inline | sub-agent`), and the spec registry block (`registry.spec.source` + `registry.spec.template`).
- Task artifacts preserve progress and validation; raw conversations remain in platform-owned stores and can be queried through `trellis mem`.
...
- Do not put team-private project rules into any public bundled skill (`trellis-meta`, `trellis-spec-bootstrap`, `trellis-session-insight`, `trellis-channel`); put project rules in `.trellis/spec/`, a project-local skill, the current task — `trellis update` will overwrite anything inside a bundled skill directory.
