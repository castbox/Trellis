...
fs.writeFileSync(path.join(sessions, "review.json"), JSON.stringify({schema_version:2,task_id:"ACTIVE-ID",lifecycle_generation:0}));
...
fs.unlinkSync(metadata);fs.writeFileSync(metadata,JSON.stringify({id:"ACTIVE-ID",title:"ACTIVE TASK",status:"in_progress"}));
